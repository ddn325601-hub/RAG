...
class SuperBizAgentApp {
    constructor() {
        ...
        this.knowledgeFiles = [];
        this.isKnowledgePanelOpen = false;
    ...
    bindEvents() {
        ...
        if (this.kbRefreshBtn) {
            this.kbRefreshBtn.addEventListener('click', () => this.loadKnowledgeFiles());
        }
    ...
    loadChatHistory(historyId) {
        ...
        if (this.chatMessages) {
            ...
            history.messages.forEach(msg => {
                this.addMessage(msg.type, msg.content, false, false, msg.references || []); // false表示不是流式，false表示不保存到历史（因为已经存在）
            });
        }
        ...
        try {
            ...
            if (data.code === 200 || data.message === 'success') {
                ...
                if (chatResponse && chatResponse.success) {
                    ...
                    const references = await this.fetchRagSources(message);
                    this.addMessage('assistant', answer, false, true, references);
                    ...
                    const references = await this.fetchRagSources(message);
                    this.addMessage('assistant', fallbackAnswer, false, true, references);
                }
            } else {
                // HTTP 成功但业务失败
    ...
    // 添加消息到聊天界面
    addMessage(type, content, isStreaming = false, saveToHistory = true, references = []) {
        ...
        if (type === 'assistant' && !isStreaming && references && references.length > 0) {
            messageContentWrapper.appendChild(this.renderSourceReferences(references));
        }
    ...
    // 格式化文件大小
    async fetchRagSources(question) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/rag/search?q=${encodeURIComponent(question)}&topK=3`);
            if (!response.ok) {
                return [];
            }
            const data = await response.json();
    ...
    renderSourceReferences(references) {
        ...
        list.className = 'source-list expanded';

        references.forEach((item, index) => {
            const metadata = this.parseMetadata(item.metadata);
            const fileName = metadata._file_name || metadata.fileName || '未知文档';
            const title = metadata.title || `片段 ${index + 1}`;
            ...
            card.innerHTML = `
                <div class="source-card-head">
                    <span class="source-file">${this.escapeHtml(fileName)}</span>
                    <span class="source-score">score ${this.escapeHtml(String(score || '-'))}</span>
                </div>
                <div class="source-title">${this.escapeHtml(title)}</div>
                <div class="source-snippet">${this.escapeHtml(this.truncateText(content, 180))}</div>
            `;
    ...
    async openKnowledgePanel() {
        ...
        if (this.kbDrawerOverlay) {
            this.kbDrawerOverlay.classList.add('open');
        }
    ...
    closeKnowledgePanel() {
        this.isKnowledgePanelOpen = false;
        if (this.kbDrawer) {
            this.kbDrawer.classList.remove('open');
        }
        if (this.kbDrawerOverlay) {
    ...
    async loadKnowledgeFiles() {
        if (this.kbStats) {
            this.kbStats.textContent = '正在读取知识库...';
        }
        if (this.kbFileList) {
        ...
        try {
            ...
            this.renderKnowledgeFiles();
        } catch (error) {
            if (this.kbStats) {
                this.kbStats.textContent = '知识库读取失败：' + error.message;
            }
        }
    }

    renderKnowledgeFiles() {
        if (!this.kbFileList || !this.kbStats) {
            return;
        }

        const totalSize = this.knowledgeFiles.reduce((sum, file) => sum + (file.fileSize || 0), 0);
        this.kbStats.textContent = `已投喂 ${this.knowledgeFiles.length} 个文件，合计 ${this.formatFileSize(totalSize)}`;
        this.kbFileList.innerHTML = '';

        if (this.knowledgeFiles.length === 0) {
            const empty = document.createElement('div');
            empty.className = 'kb-empty';
            empty.textContent = '还没有知识库文件。点击“上传资料”添加 Markdown 或 TXT。';
            this.kbFileList.appendChild(empty);
            return;
        }
        ...
        this.knowledgeFiles.forEach(file => {
            ...
            const time = file.lastModified ? new Date(file.lastModified).toLocaleString() : '-';
            item.innerHTML = `
            ...
            `;
            this.kbFileList.appendChild(item);
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.superBizApp = new SuperBizAgentApp();
});
